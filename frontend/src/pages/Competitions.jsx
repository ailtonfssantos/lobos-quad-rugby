...
export default function Competitions() {
  ...
  // --- FUNCIONES DE FECHA Y ESTADO (Tu código original intacto) ---
  ...
  const TeamLogo = ({ logo, name, size = "large" }) => {
    ...
    if (!logo) return <div className={`${dimensions} rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center`}><span className="text-zinc-600 text-[10px] font-bold uppercase text-center px-2">{name?.substring(0, 3)}</span></div>;
    return (
        ...
        <div className="p-5 md:p-8">
          <div className="text-center mb-6 md:mb-8">
            <div className="flex items-center justify-center gap-2 text-zinc-500">
              ...
              {partido?.horario && (<><span className="text-zinc-700">·</span><span className="text-red-500 text-sm md:text-base font-display font-bold tracking-wider">{partido.horario}</span></>)}
            </div>
          </div>
          <div className="hidden sm:grid grid-cols-[1fr_auto_1fr] items-center gap-6 md:gap-10">
            <div className="flex flex-col items-center text-center"><TeamLogo logo={home.logo} name={home.name} /><h3 className="mt-4 text-sm md:text-base font-bold uppercase tracking-wide text-white max-w-[180px]">{home.name}</h3></div>
            <div className="flex flex-col items-center min-w-[130px]">{status === "FINALIZADO" && hasScore ? <Score homeScore={partido.lobosScore} awayScore={partido.rivalScore} homeName={home.name} awayName={away.name} /> : <span className="font-display text-lg md:text-xl text-zinc-700">VS</span>}</div>
            <div className="flex flex-col items-center text-center"><TeamLogo logo={away.logo} name={away.name} /><h3 className="mt-4 text-sm md:text-base font-bold uppercase tracking-wide text-white max-w-[180px]">{away.name}</h3></div>
          </div>
          <div className="sm:hidden">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
              <div className="flex flex-col items-center text-center"><TeamLogo logo={home.logo} name={home.name} size="small" /><p className="mt-3 text-[10px] font-bold uppercase tracking-wide text-white leading-tight max-w-[100px]">{home.name}</p></div>
              <div className="flex flex-col items-center px-2">{status === "FINALIZADO" && hasScore ? <div className="flex items-center gap-2"><span className="font-display text-2xl font-bold text-white">{partido.lobosScore}</span><span className="text-zinc-700">-</span><span className="font-display text-2xl font-bold text-white">{partido.rivalScore}</span></div> : <span className="font-display text-sm text-zinc-700">VS</span>}</div>
              <div className="flex flex-col items-center text-center"><TeamLogo logo={away.logo} name={away.name} size="small" /><p className="mt-3 text-[10px] font-bold uppercase tracking-wide text-white leading-tight max-w-[100px]">{away.name}</p></div>
            </div>
          </div>
  ...
  if (loading) return <div className="min-h-screen bg-zinc-950 flex items-center justify-center"><div className="text-center"><div className="w-8 h-8 border-2 border-zinc-800 border-t-red-500 rounded-full animate-spin mx-auto mb-4"></div><p className="text-zinc-500 text-xs uppercase tracking-[0.2em]">Cargando competiciones...</p></div></div>;
  if (error) return <div className="min-h-screen bg-zinc-950 text-white"><section className="min-h-[60vh] flex items-center justify-center px-4"><div className="text-center max-w-lg"><div className="w-14 h-14 border border-red-500/20 bg-red-500/5 flex items-center justify-center mx-auto mb-6"><Icon path="M12 9v3.75m0 3h.007M10.29 3.86l-7.82 13.5A1.5 1.5 0 003.77 19.6h16.46a1.5 1.5 0 001.3-2.24l-7.82-13.5a1.5 1.5 0 00-2.6 0z" className="w-6 h-6 text-red-500" /></div><h1 className="font-display text-3xl text-white mb-3">No se pudieron cargar las competiciones</h1><p className="text-zinc-500 text-sm leading-relaxed">Se ha producido un error al conectar con el calendario. Inténtalo de nuevo más tarde.</p></div></section></div>;
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <main className="max-w-5xl mx-auto px-4 py-12 md:py-20">
        {jornadasFiltradas.length === 0 ? (
          <div className="py-20 text-center">
            ...
            <p className="text-zinc-600 italic text-sm">No hay jornadas programadas para esta temporada.</p>
          </div>
        ) : (
                ...
                <div className={`p-5 md:p-8 ${jornada.bannerUrl ? "pt-5 md:pt-7" : ""}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-7 md:mb-8">
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 shrink-0 bg-zinc-950 border border-zinc-800 flex items-center justify-center"><Icon path="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" className="w-4 h-4 text-red-500" /></div>
                      <div><p className="text-[9px] uppercase tracking-[0.15em] text-zinc-600 mb-1">Ciudad</p><p className="text-sm text-zinc-200 font-medium">{jornada.ciudad || "Por confirmar"}</p></div>
                    </div>
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 shrink-0 bg-zinc-950 border border-zinc-800 flex items-center justify-center"><Icon path="M3 21h18M5 21V9l7-5 7 5v12M9 21v-6h6v6" className="w-4 h-4 text-red-500" /></div>
                      <div><p className="text-[9px] uppercase tracking-[0.15em] text-zinc-600 mb-1">Local</p><p className="text-sm text-zinc-200 font-medium">{jornada.pabellon || "Por confirmar"}</p></div>
                    </div>
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 shrink-0 bg-zinc-950 border border-zinc-800 flex items-center justify-center"><Icon path="M8 7V3m8 4V3M4 11h16M5 5h14a1 1 0 011 1v13a1 1 0 01-1 1H5a1 1 0 01-1-1V6a1 1 0 011-1z" className="w-4 h-4 text-red-500" /></div>
                      <div><p className="text-[9px] uppercase tracking-[0.15em] text-zinc-600 mb-1">Fecha</p><p className="text-sm text-zinc-200 font-medium">{formatFullDate(jornada)}</p></div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    {Array.isArray(jornada.partidos) && jornada.partidos.length > 0 ? (
                      jornada.partidos.map((partido, index) => <MatchCard key={partido.id || `${jornada.id}-${index}`} jornada={jornada} partido={partido} index={index} />)
                    ) : (
                      <div className="py-10 text-center border border-zinc-800 bg-zinc-950"><p className="text-zinc-600 text-xs uppercase tracking-wider">No hay partidos disponibles para esta jornada.</p></div>
                    )}
                  </div>
                </div>
              </section>
      ...
      <section className="py-16 md:py-24 bg-red-600 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-sm md:text-lg mb-8 md:mb-10 text-red-100 font-light max-w-2xl mx-auto">Sigue nuestros próximos partidos, consulta los resultados y acompáñanos durante toda la temporada.</p>
          <Link to="/entrenamientos" className="inline-flex items-center gap-3 px-8 md:px-10 py-4 md:py-5 bg-zinc-950 text-white font-bold text-xs uppercase tracking-[0.15em] hover:bg-black transition-all duration-300 shadow-2xl">
            Ver Calendario y Ubicación<Icon path="M5 12h14m-6-6l6 6-6 6" className="w-4 h-4" />
          </Link>
        </div>
      </section>
    </div>
