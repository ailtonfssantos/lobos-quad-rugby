...
export default function Jornadas() {
  ...
  const initialPartido = { 
    rival: '', diaSemana: 'Sábado', horario: '', youtubeLink: '', 
    status: 'PROGRAMADO', lobosScore: '', rivalScore: '',
  ...
  const openModal = (jornada = null) => {
    if (jornada) {
      ...
      setFormData({
        ...
        partidos: partidosRellenos.map(p => ({ 
          ...initialPartido, ...p, 
          lobosScore: p.lobosScore ?? '', rivalScore: p.rivalScore ?? '' 
        }))
      });
    } else {
      setEditingId(null);
  ...
  const updatePartido = (index, field, value) => {
    ...
    if (field === 'equipoVisitanteNombre') nuevosPartidos[index].rival = value;
    if (field === 'equipoVisitanteLogo') nuevosPartidos[index].rivalLogo = value;
  ...
  const handleSubmit = async (e) => {
    ...
    // Limpa partidos vazios
    const partidosValidos = formData.partidos.filter(p => p.equipoVisitanteNombre || p.rival || p.horario);
            ...
            <tbody className="divide-y divide-zinc-800">
              {displayedJornadas.map((j) => (
                <tr key={j.id} className="hover:bg-zinc-800/30 transition-colors">
                        ...
                        return (
                          <div key={idx} className="text-xs">
                            ...
                            <div className="text-white">
                              {local} <span className="text-zinc-700 mx-1">vs</span> {visitante}
                            </div>
                            {p.status === 'FINALIZADO' && (
                              <span className="text-green-500">{p.lobosScore} - {p.rivalScore}</span>
                            )}
                          </div>
                        );
                      })}
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-full ${j.isActive ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-zinc-800 text-zinc-500 border border-zinc-700'}`}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="md:col-span-2">
                  ...
                  <input type="text" value={formData.fechas} onChange={(e) => setFormData({...formData, fechas: e.target.value})} required placeholder="10 y 11 de octubre de 2026" className="w-full bg-zinc-950 border border-zinc-700 text-white px-4 py-3 rounded-sm focus:border-red-600 outline-none" />
                </div>
                
                {/* ✅ BANNER UPLOAD LIMPO (SEM O INPUT DE TEXTO) */}
                <div className="md:col-span-2">
                  <label className="block text-zinc-400 text-xs uppercase tracking-widest mb-2">Banner Promocional</label>
                  <div className="flex items-center gap-4">
                    {formData.bannerUrl && (
                      <img src={getImageUrl(formData.bannerUrl)} alt="Banner" className="h-16 w-auto border border-zinc-700 rounded-sm object-cover" />
                    )}
                    <label className="flex items-center gap-2 px-4 py-2 bg-zinc-950 border border-zinc-700 hover:border-red-600 text-zinc-300 cursor-pointer rounded-sm text-sm transition-colors">
                ...
                <div className="space-y-4">
                  {formData.partidos.map((p, idx) => (
                    <div key={idx} className="bg-zinc-950 border border-zinc-800 p-4 rounded-sm">
                      <p className="text-zinc-500 text-xs uppercase tracking-widest mb-3">Partido {idx + 1}</p>
                      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-3">
                        <input type="text" placeholder="Rival (Ej: Adapta)" value={p.equipoVisitanteNombre || p.rival} onChange={(e) => updatePartido(idx, 'equipoVisitanteNombre', e.target.value)} className="bg-zinc-900 border border-zinc-700 text-white px-3 py-2 rounded-sm text-sm outline-none focus:border-red-600 md:col-span-2" />
                        <select value={p.diaSemana} onChange={(e) => updatePartido(idx, 'diaSemana', e.target.value)} className="bg-zinc-900 border border-zinc-700 text-white px-3 py-2 rounded-sm text-sm outline-none">
                          <option>Sábado</option><option>Domingo</option><option>Viernes</option>
                        </select>
                      ...
                      <div className="mb-3">
                        <input type="url" placeholder="Enlace de YouTube (Opcional)" value={p.youtubeLink} onChange={(e) => updatePartido(idx, 'youtubeLink', e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 text-white px-3 py-2 rounded-sm text-sm outline-none focus:border-red-600" />
                      </div>
                      {p.status === 'FINALIZADO' && (
