...
// Admin vê todos.
// Público vê apenas eventos activos.
// =========================================================
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `✅ Evento ${id} marcado como finalizado`
      );
...
// La imagen ya debe estar subida a Cloudinary.
// =========================================================
router.post(
  '/:id/fotos',
  authMiddleware,
  async (req, res) => {
    try {
      ...
      console.error(
        '❌ Error al añadir foto al evento:',
        error
      );
...
// Elimina también la imagen de Cloudinary cuando existe publicId.
// =========================================================
router.delete(
  '/:id/fotos/:fotoId',
  authMiddleware,
  async (req, res) => {
    try {
      ...
      const foto =
        await prisma.eventoFoto.findFirst({
          where: {
            id: parseInt(
              fotoId,
              10
      ...
      if (foto.publicId) {
        try {
          ...
          console.error(
            '⚠️ Error al eliminar imagen de Cloudinary:',
            cloudinaryError
          );
        }
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `🗑️ Eliminando permanentemente evento ${id}`
      );
